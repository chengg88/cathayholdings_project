...
file_handler = TimedRotatingFileHandler(
    './var/log/'+log_filename, when="midnight", interval=1, encoding="utf-8", backupCount=9)
...
class MLpipeline:
    ...
    def img_processes(self, val_size, test_size, shuffle_dataset, random_seed, batch_size):
        IMG = image_manager(cfg["img"]['img_path'],
                            cfg["img"]['df_path'], logger)
        # ETL = ETL()
        IMG.get_image_info()
        dataset = IMG.creat_dataset(height=32, width=32,
                                    transform=transforms.Compose([
                                        # 將圖片從原先大小28x28改成LeNet可以接受的輸入大小32x32
                                        transforms.Resize((32, 32)),
                                        # 轉換成tensor並且將像素範圍(range)從[0, 255]改到[0,1]
                                        transforms.ToTensor(),
